...
export class ResourceService extends BaseService<ResourceData> {
    ...
    constructor(protected http: Http, protected cacheEventBus: CacheEventBus) {
        super(http, ResourceData, {
            ...
            cacheConfig: {
                tag: Constants.RESOURCE_SERVICE_CACHE_TAG,
                isCacheable: false,
                relatedTags: [Constants.ITEM_SERVICE_CACHE_TAG]
            }
        }, cacheEventBus);
    }
    ...
    getResourceStream(pid: string, conversion: string): Observable<ResourceData> {
        ...
        const cachedData: any = this.getCachedData(url, null, { tag: Constants.RESOURCE_SERVICE_CACHE_TAG, isCacheable: true });
        if (cachedData) {
            ...
            return this.http
                .get(url, reqOptions)
                .map((res: Response) => {
                    const data = ResourceDataUtility.fromResponse(res);
                    if (res.status === 200) {
                        this.setCachedData(url, null, data, { tag: Constants.RESOURCE_SERVICE_CACHE_TAG, isCacheable: true });
                    }
